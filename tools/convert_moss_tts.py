...
def remap_backbone_name(name: str) -> str | None:
    """MossTTSDelay / MossTTSD tensor name → Qwen3ForCausalLM convention.
    Returns None when the tensor is not a backbone tensor.

    MossTTSDelay uses: language_model.*, lm_heads.0.weight
    MossTTSD uses:     model.language_model.*, (lm_head tied with embed)
    """
    # MossTTSD has a model. prefix; strip it for uniform handling
...
def extract_qwen3_backbone(moss_dir: Path, out_dir: Path) -> dict:
    ...
    # Qwen3 config.
    # MossTTSDelay nests the backbone config under "language_config";
    # MossTTSD is flat (the top-level config IS the Qwen3 config plus
    # TTSD-specific extras that we strip).
    if "language_config" in moss_config:
        lang = dict(moss_config["language_config"])
    else:
        # Flat TTSD config: keep Qwen3 architecture fields, drop TTSD-only keys.
        _TTSD_STRIP = {
            "channels", "speech_pad_token", "speech_token_range",
            "speech_vocab_size", "vocab_size_list", "pad_token",
            "auto_map",
        }
        lang = {k: v for k, v in moss_config.items() if k not in _TTSD_STRIP}
...
def collect_audio_extras(moss_dir: Path, n_vq: int):
    ...
    Handles both MossTTSDelay (emb_ext.*, lm_heads.*) and MossTTSD
    (model.embedding_list.*, no separate heads — heads are tied to embeddings)
    naming conventions.

    For MossTTSD the audio heads are *tied* to the embedding tables, so we
    materialise them as duplicate tensors under ``moss.audio_head.N.weight``
    so the C++ loader/inference can treat them uniformly. The size cost is
    small (n_vq × audio_vocab × hidden_size × 2 bytes ≈ tens of MB).
    """
    weight_map = load_safetensors_index(moss_dir)
    ...
    # Track which codebooks we saw embeddings for, so we can materialise
    # tied heads for any codebook that lacks an explicit lm_head.
    embedded_codebooks: set[int] = set()
    explicit_heads: set[int] = set()
    materialised: list = []   # (gguf_name, ndarray)
        ...
        with STShard(moss_dir / shard) as sf:
            for tname in sorted(by_shard[shard]):
                if tname.startswith("emb_ext.") and tname.endswith(".weight"):
                    ...
                    embedded_codebooks.add(idx)
                    materialised.append((f"moss.audio_embed.{idx}.weight", sf.get_f16(tname)))
                elif tname.startswith("model.embedding_list.") and tname.endswith(".weight"):
                    idx = int(tname.split(".")[-2])
                    embedded_codebooks.add(idx)
                    arr = sf.get_f16(tname)
                    materialised.append((f"moss.audio_embed.{idx}.weight", arr))
                    # TTSD ties the output head to the embedding table; emit a
                    # duplicate tensor so the C++ side finds moss.audio_head.N.
                    materialised.append((f"moss.audio_head.{idx}.weight", arr))
                elif tname.startswith("lm_heads.") and tname.endswith(".weight"):
                    idx = int(tname.split(".")[1])
                    if idx == 0:
                        continue # already mapped to lm_head.weight by stage 1
                    explicit_heads.add(idx - 1)
                    yield f"moss.audio_head.{idx-1}.weight", sf.get_f16(tname)

    # Yield the non-lm_heads tensors we accumulated.
    embed_yielded = set()
    for name, arr in materialised:
        # Deduplicate: if the same name appears twice (shouldn't, but be safe)
        # keep the first occurrence.
        if name in embed_yielded:
            continue
        embed_yielded.add(name)
        yield name, arr
...
def write_moss_sidecar(out_gguf: Path,
    ...
    # ── 1. emit MOSS KV ─────────────────────────────────────────────────────
    # MossTTSDelay uses n_vq / audio_vocab_size / audio_*_token_id.
    # MossTTSD    uses channels / speech_vocab_size / speech_pad_token /
    #              speech_token_range / vocab_size_list. We normalise both to
    #              the moss.* KV namespace the C++ loader expects.
    n_vq             = int(moss_config.get("n_vq", moss_config.get("channels", 32)))
    # audio_vocab_size: prefer audio_vocab_size, fall back to speech_vocab_size-1
    # (speech_vocab_size includes pad), final fallback to per-codebook vocab_size_list.
    vocab_size_list = moss_config.get("vocab_size_list")
    if "audio_vocab_size" in moss_config:
        audio_vocab_size = int(moss_config["audio_vocab_size"])
    elif "speech_vocab_size" in moss_config:
        audio_vocab_size = int(moss_config["speech_vocab_size"]) - 1  # exclude pad
    elif isinstance(vocab_size_list, list) and len(vocab_size_list) > 1:
        audio_vocab_size = int(vocab_size_list[1]) - 1
    else:
        audio_vocab_size = 1024
    audio_pad_code   = int(moss_config.get("audio_pad_code",
                                           moss_config.get("speech_pad_token",
                                                           audio_vocab_size)))
    ...
    # Special tokens — TTSD embeds audio tokens in the main vocab via
    # speech_token_range=[start, end]; the delay models use explicit
    # audio_*_token_id fields. We expose both views so the C++ side can pick.
    if "speech_token_range" in moss_config:
        sr_lo, sr_hi = moss_config["speech_token_range"]
        writer.add_uint32("moss.token.audio_start", int(sr_lo))
        writer.add_uint32("moss.token.audio_end",   int(sr_hi))
    else:
    ...
    # TTSD-specific flags so the C++ loader knows what it's dealing with.
    writer.add_bool("moss.tied_audio_heads",
                    bool(moss_config.get("tie_word_embeddings", False)))
...
def main():
    ...
    ap.add_argument("--llama-cpp-dir", default=None,
                    help="Path to a built llama.cpp tree (default: auto-detect third_party/llama.cpp)")
    ...
    # Auto-detect llama.cpp directory if not explicitly provided.
    if args.llama_cpp_dir is None:
        script_dir = Path(__file__).resolve().parent
        for candidate in [
            script_dir.parent / "third_party" / "llama.cpp",
            Path("/usr/local/src/llama.cpp"),
            Path("/devel/tools/llama.cpp"),
        ]:
            if (candidate / "convert_hf_to_gguf.py").exists():
                args.llama_cpp_dir = str(candidate)
                break
        if args.llama_cpp_dir is None:
            ap.error("--llama-cpp-dir not given and auto-detection failed. "
                     "Pass --llama-cpp-dir /path/to/llama.cpp explicitly.")
