...
unique_ptr<tts_generation_runner> runner_from_file(const char * fname, int n_threads,
    ...
    if (use_mmap) {
        ...
        for (ggml_tensor * cur_p = ggml_get_first_tensor(weight_ctx); cur_p; cur_p = ggml_get_next_tensor(weight_ctx, cur_p)) {
        ggml_tensor & cur = *cur_p;
            GGML_ASSERT(i < n);
