...
// ── Missing GGML ops shims (not in upstream llama.cpp) ───────────────────

// ggml_mod(x, s): modulo by scalar. Used in sin_gen with s=1.0 (fractional part).
static ggml_tensor* ggml_mod(ggml_context* ctx, ggml_tensor* x, float s) {
    if (s == 1.0f) {
        return ggml_sub(ctx, x, ggml_floor(ctx, x));
    }
    // General case: would need scalar tensor support; not used by Kokoro.
    return ggml_sub(ctx, x, ggml_floor(ctx, x));
}

// ggml_upscale_linear: 1D linear-interpolation upsample by integer factor.
// TODO: implement true linear interp. Nearest-neighbour approximation for now.
static ggml_tensor* ggml_upscale_linear(ggml_context* ctx, ggml_tensor* x, int scale) {
    return ggml_upscale_ext(ctx, x, x->ne[0] * scale, x->ne[1], x->ne[2], x->ne[3], GGML_SCALE_MODE_NEAREST);
}
...
static struct ggml_tensor * build_ada_residual_conv(ggml_context * ctx, struct ggml_tensor * x, ada_residual_conv_block * block, struct ggml_tensor * style, struct ggml_tensor * sqrt_tensor) {
	...
	if (block->pool) {
		cur = ggml_conv_transpose_1d(ctx, block->pool, cur, 2, 1, 1);
...
static struct ggml_tensor * build_generator(ggml_context * ctx, kokoro_model * model, kokoro_context * kctx, struct ggml_tensor * x, struct ggml_tensor * style, struct ggml_tensor * f0_curve, kokoro_generator* generator, int sequence_length, struct ggml_tensor * window_sq_sum, ggml_cgraph * gf) {
	...
	for (int i = 0; i < generator->ups.size(); i++) {
		...
		cur = ggml_add(ctx, ggml_conv_transpose_1d(ctx, generator->ups[i]->upsample_weight, ggml_cont(ctx, ggml_transpose(ctx, cur)), generator->ups[i]->stride, generator->ups[i]->padding, 1), generator->ups[i]->upsample_bias);
...
void kokoro_duration_runner::run(kokoro_ubatch & batch) {
    ...
    struct ggml_tensor * hidden_states = ggml_graph_node(gf, -22 - 52 * (int)batch.n_tokens);
...
void kokoro_runner::assign_weight(const char * name, ggml_tensor & tensor) {
    ...
    GGML_ASSERT(name_sv.rfind("kokoro.", 0) == 0);
...
std::vector<std::string_view> kokoro_runner::list_voices() {
    ...
    for (const auto & kv : model->voices) {
        result.push_back(std::string_view{kv.first});
    }
    return result;
}
