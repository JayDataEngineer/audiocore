...
void word_phonemizer::add_rule(std::vector<std::string> keys, std::string phoneme) {
	...
	for (int i = 0; i < keys.size(); i++) {
		if (current_rule) {
			if (!current_rule->rules.count(keys[i])) {
...
bool phonemizer::process_word(corpus* text, std::string* output, std::string word, conditions* flags, bool has_accent) {
	...
	} else if (word.length() > 1 && !small_english_words.count(to_lower(word)) && can_be_roman_numeral(word) && is_all_upper(word) && handle_roman_numeral(text, output, flags)) {
		return true;
