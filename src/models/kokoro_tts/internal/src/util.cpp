...
// ── STFT/iSTFT op stubs (mmwillet GGML fork has these natively) ──────────
// Simple CPU-only implementations for Kokoro's source signal generation.
// These create tensor views that are computed lazily — for now we provide
// minimal stubs that produce the right output shape. Full DSP-correct
// implementations will be added in a follow-up.

struct ggml_tensor * ggml_stft(struct ggml_context * ctx,
                                struct ggml_tensor * a,
                                struct ggml_tensor * window,
                                size_t n_fft, size_t hop, bool abs_and_angle) {
    // Output shape: [n_fft/2+1, num_frames] (one-sided)
    size_t num_frames = (a->ne[0] - n_fft) / hop + 1;
    size_t out_cols = abs_and_angle ? 2 : 1;
    struct ggml_tensor * result = ggml_new_tensor_2d(ctx, GGML_TYPE_F32,
                                                      n_fft / 2 + 1,
                                                      num_frames * out_cols);
    ggml_set_name(result, "stft_output");
    return result;
}

struct ggml_tensor * ggml_istft(struct ggml_context * ctx,
                                 struct ggml_tensor * a,
                                 struct ggml_tensor * window,
                                 size_t n_fft, size_t hop, bool abs_and_angle) {
    // Output: reconstructed signal, length = (a->ne[1]-1) * hop + n_fft
    size_t num_frames = abs_and_angle ? a->ne[1] / 2 : a->ne[1];
    size_t signal_len = (num_frames - 1) * hop + n_fft;
    struct ggml_tensor * result = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, signal_len);
    ggml_set_name(result, "istft_output");
    return result;
}
