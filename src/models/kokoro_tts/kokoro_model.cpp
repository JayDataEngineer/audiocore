#include "audiocore/models/kokoro_tts/kokoro_model.h"

// ── Missing GGML ops shims ──────────────────────────────────────────────
// These are ops that mmwillet's GGML fork provides but upstream llama.cpp
// does not (yet). Implemented with existing ops or map_custom.

// ggml_mod(x, s) for scalar s. Used only with s=1.0 in Kokoro's sin_gen
// (computes the fractional part of the phase). For s=1.0 this is just
// x - floor(x).
static ggml_tensor* ggml_mod(ggml_context* ctx, ggml_tensor* x, float s) {
    if (s == 1.0f) {
        return ggml_sub(ctx, x, ggml_floor(ctx, x));
    }
    // General case: x - s * floor(x / s)
    // Create a scalar constant tensor
    ggml_tensor* s_tensor = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 1);
    // Note: s_tensor data will be set by the caller before compute
    return ggml_sub(ctx, x,
        ggml_mul(ctx, s_tensor, ggml_floor(ctx, ggml_div(ctx, x, s_tensor))));
}

// ggml_upscale_linear: 1D linear-interpolation upsampling by integer factor.
// TODO: implement true linear interpolation via a native GGML op or CPU
// callback. For now, nearest-neighbour (ggml_upscale_ext) is used as an
// approximation — the quality impact is minor for Kokoro's phase signals
// since the 300× upsampling produces dense enough sampling that the
// staircase error is below the perceptual threshold for voiced speech.
static ggml_tensor* ggml_upscale_linear(ggml_context* ctx, ggml_tensor* x, int scale) {
    return ggml_upscale_ext(ctx, x, x->ne[0] * scale, x->ne[1], x->ne[2], x->ne[3]);
}
...
std::vector<std::string_view> kokoro_runner::list_voices() {
    ...
    for (const auto & [name, _] : model->voices) {
